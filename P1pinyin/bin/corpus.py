            ...
            for line in f:
                ...
                for i, c in enumerate(str):
                    if c != ' ':
                        ...
                        # if the character before is also in the dict, count the bi-frequency c(wi, wj)
                        if last1 != ' ':
                            if last1 in self.bi_gram[char]:
                                self.bi_gram[char][last1] += 1
                            else:
                                self.bi_gram[char][last1] = 1
                            if last2 != ' ':
                                lastword = last2 + last1
                                if lastword in self.tri_gram[char]:
                                    self.tri_gram[char][lastword] += 1
                                else:
                                # new
                                    self.tri_gram[char][lastword] = 1
                        last2 = last1
